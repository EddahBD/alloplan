...
const LOCATIONS = [
  ...
  "Tanga", "Morogoro", "Zanzibar", "Kilimanjaro",
];
...
function VendorMapPin({
  ...
  return (
    ...
    >
      ...
      {selected && (
        <Text style={[styles.pinLabel, { color: "#fff", fontFamily: "Poppins_600SemiBold" }]} numberOfLines={1}>
...
export default function VendorMapScreen() {
  ...
  // Pseudo-random but stable positions for pins based on vendor id
  const getPinPosition = (vendor: VendorCard, idx: number) => {
    const seed = vendor.id * 7 + idx * 13;
    const x = 20 + (seed % 260);
    const y = 30 + ((seed * 3) % 220);
    return { x, y };
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
          ...
          {LOCATIONS.map((loc) => (
            ...
            >
              <Text style={[styles.locationText, {
                ...
                fontFamily: selectedLocation === loc ? "Poppins_600SemiBold" : "Poppins_400Regular",
              }]}>
                {loc}
              </Text>
            </TouchableOpacity>
      ...
      {/* Map area */}
      <View style={[styles.mapArea, { backgroundColor: colors.muted, borderBottomColor: colors.border }]}>
        {/* Map background grid */}
        ...
        {/* Location label */}
        <View style={[styles.locationLabel, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Ionicons name="location" size={14} color={colors.primary} />
          <Text style={[styles.locationLabelText, { color: colors.foreground, fontFamily: "Poppins_600SemiBold" }]}>
            {selectedLocation}
          </Text>
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.mapEmpty}>
            <Text style={[styles.mapEmptyText, { color: colors.mutedForeground, fontFamily: "Poppins_400Regular" }]}>
              No vendors in {selectedLocation}
            </Text>
          </View>
        ) : (
          vendors.map((vendor, idx) => {
            const pos = getPinPosition(vendor, idx);
            return (
              <VendorMapPin
                key={vendor.id}
                vendor={vendor}
      ...
      {selectedVendor && (
        <TouchableOpacity
          ...
          style={[styles.selectedCard, { backgroundColor: colors.card, borderColor: colors.primary }]}
        >
          <View style={[styles.selectedCover, { backgroundColor: colors.secondary, borderRadius: 10, overflow: "hidden" }]}>
            {selectedVendor.coverImage ? (
              <Image source={{ uri: selectedVendor.coverImage }} style={styles.selectedCoverImg} />
            ) : (
              <View style={[styles.selectedCoverPlaceholder, { backgroundColor: colors.secondary }]}>
          ...
          <View style={styles.selectedInfo}>
            <View style={styles.selectedNameRow}>
              <Text style={[styles.selectedName, { color: colors.foreground, fontFamily: "Poppins_600SemiBold" }]} numberOfLines={1}>
                {selectedVendor.businessName || selectedVendor.ownerName}
              </Text>
              {selectedVendor.verified && <Ionicons name="checkmark-circle" size={14} color={colors.primary} />}
            </View>
            {selectedVendor.businessType && (
              <Text style={[styles.selectedType, { color: colors.mutedForeground, fontFamily: "Poppins_400Regular" }]}>
                {selectedVendor.businessType}
              </Text>
            )}
            <View style={styles.selectedMeta}>
              <Ionicons name="star" size={12} color={colors.accent} />
              <Text style={[styles.selectedRating, { color: colors.foreground, fontFamily: "Poppins_500Medium" }]}>
                {selectedVendor.rating ? selectedVendor.rating.toFixed(1) : "New"}
              </Text>
              {selectedVendor.minPrice && (
                <Text style={[styles.selectedPrice, { color: colors.primary, fontFamily: "Poppins_600SemiBold" }]}>
      ...
      <FlatList
        ...
        contentContainerStyle={[styles.listRow, { paddingBottom: Platform.OS === "web" ? 24 : insets.bottom + 16 }]}
        ListHeaderComponent={
          <Text style={[styles.listHeader, { color: colors.foreground, fontFamily: "Poppins_600SemiBold" }]}>
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            style={[styles.listCard, {
              ...
              borderColor: selectedVendor?.id === item.id ? colors.primary : colors.border,
              borderRadius: colors.radius,
            }]}
          >
            <View style={[styles.listCover, { backgroundColor: colors.secondary, borderRadius: colors.radius, overflow: "hidden" }]}>
              {item.coverImage ? (
            ...
            <Text style={[styles.listName, { color: colors.foreground, fontFamily: "Poppins_600SemiBold" }]} numberOfLines={1}>
              {item.businessName || item.ownerName}
            </Text>
            {item.businessType && (
              <Text style={[styles.listType, { color: colors.mutedForeground, fontFamily: "Poppins_400Regular" }]} numberOfLines={1}>
                {item.businessType}
              </Text>
            )}
            <View style={styles.listRating}>
              <Ionicons name="star" size={10} color={colors.accent} />
              <Text style={[styles.listRatingText, { color: colors.foreground, fontFamily: "Poppins_500Medium" }]}>
...
const styles = StyleSheet.create({
  ...
  locationChip: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 999, borderWidth: 1 },
  locationText: { fontSize: 13 },
  mapArea: { height: 300, position: "relative", borderBottomWidth: 1, overflow: "hidden" },
  ...
  locationLabel: { position: "absolute", top: 12, right: 12, flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20, borderWidth: 1 },
  locationLabelText: { fontSize: 12 },
  mapLoading: { flex: 1, alignItems: "center", justifyContent: "center" },
  mapEmpty: { flex: 1, alignItems: "center", justifyContent: "center" },
  mapEmptyText: { fontSize: 14 },
  pin: { position: "absolute", flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 6, borderRadius: 20, borderWidth: 1.5, maxWidth: 140 },
  pinEmoji: { fontSize: 14 },
  pinLabel: { fontSize: 11, flexShrink: 1 },
  selectedCard: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14, borderTopWidth: 2, borderBottomWidth: 1 },
